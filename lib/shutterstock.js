...
function biasQueryTowardIcons(query) {
  const cleaned = query
    ...
    .trim();
  // OR the subject words (website-like behavior): sheets are tagged with one
  // theme each, so ANDing "heart clock" excludes nearly every set
  const words = cleaned.split(/\s+/).filter(Boolean);
  const subject = words.length > 1 ? `(${words.join(" OR ")})` : cleaned;
  return `${subject} line icons`.trim();
}
