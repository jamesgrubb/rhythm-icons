...
// Terms that bias results toward stroke-based icon sets (the only assets the
// library can ingest) and away from illustrations/lettering. "editable stroke"
// is the standard Shutterstock tag for non-outlined line icon sets.
const ICON_BIAS_TERMS = ["line icon", "editable stroke", "outline"];

function biasQueryTowardIcons(query) {
  const lower = query.toLowerCase();
  const missing = ICON_BIAS_TERMS.filter((t) => !lower.includes(t));
  return missing.length ? `${query} ${missing.join(" ")}` : query;
}
