...
// Require the exact phrase "editable stroke" (quoted = phrase match in
// Shutterstock search) so results are stroke-based icon sets the library can
// ingest. Loose keywords only bias relevance and still let illustrations
// through; the quoted phrase actually filters.
function biasQueryTowardIcons(query) {
  const cleaned = query.replace(/"?editable stroke"?/gi, "").trim();
  return `"editable stroke" ${cleaned}`.trim();
}
