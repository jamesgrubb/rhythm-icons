...
// Paint regions white (mask out branding/panels) so the naming pass sees only
// the real icons. Regions are [ymin,xmin,ymax,xmax] normalized 0-1000.
function maskRegions(pngBuffer, regions) {
  if (!regions || !regions.length) return pngBuffer;
  ...
  for (const [ymin, xmin, ymax, xmax] of regions) {
    ...
    for (let y = y0; y < y1; y++) {
      for (let x = x0; x < x1; x++) {
        const i = ((y * width) + x) << 2;
        png.data[i] = png.data[i + 1] = png.data[i + 2] = 255; png.data[i + 3] = 255;
      }
    }
  }
...
/**
 * Name already-segmented icons: given a grid render of the extracted icons
 * (left→right, top→bottom, `cols` per row) return one {label, tags} per icon
 * in grid order. Gemini sees the exact icon it names, so names always match.
 * @returns [{label, tags}] of length `count` (best effort)
 */
async function nameIconGrid(pngBuffer, count, cols) {
  const genAI = getClient();
  const model = genAI.getGenerativeModel({
    model: MODEL,
    generationConfig: { responseMimeType: "application/json", temperature: 0, maxOutputTokens: 8192 },
  });
  const prompt = `This image is a grid of ${count} individual line icons, arranged left-to-right then top-to-bottom, ${cols} per row, on a light background.
For EACH icon, in that exact order, give a short name and search keywords.
Return ONLY a JSON array of EXACTLY ${count} objects, in grid order:
{ "label": "short-kebab-name", "tags": ["keyword", ...] }
- label: 1-3 lowercase words hyphen-separated, describing what the icon depicts (e.g. "bar-chart", "person-presenting")
- tags: 3-6 lowercase single-word search keywords (what it depicts + what it's used for + synonyms)
- Output exactly ${count} entries, one per icon, even if some look similar.`;
  let arr = [];
  try {
    const r = await model.generateContent([{ inlineData: { mimeType: "image/png", data: pngBuffer.toString("base64") } }, prompt]);
    const p = parseJsonArray(r.response.text());
    if (Array.isArray(p)) arr = p;
  } catch (e) {
    console.warn("[Gemini] icon-grid naming failed:", e.message);
  }
  return arr.map((d) => ({
    label: String(d.label || "icon").toLowerCase().trim().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "icon",
    tags: (Array.isArray(d.tags) ? d.tags : []).map((t) => String(t).toLowerCase().trim().replace(/[^a-z0-9-]+/g, "").slice(0, 30)).filter(Boolean).slice(0, 8),
  }));
}

/**
 * Name already-segmented icons by sending each as its OWN image, in order,
 * batched. Gemini names image 1, image 2, … — no grid to miscount, so names
 * can't drift relative to the icons.
 * @param {Buffer[]} pngBuffers one small PNG per extracted icon, in order
 * @returns [{label, tags}] aligned 1:1 with pngBuffers (best effort)
 */
async function nameIconsOrdered(pngBuffers) {
  const genAI = getClient();
  const model = genAI.getGenerativeModel({
    model: MODEL,
    generationConfig: { responseMimeType: "application/json", temperature: 0, maxOutputTokens: 8192 },
  ...
  const clean = (d) => ({
    label: String(d && d.label || "icon").toLowerCase().trim().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "icon",
    tags: (Array.isArray(d && d.tags) ? d.tags : []).map((t) => String(t).toLowerCase().trim().replace(/[^a-z0-9-]+/g, "").slice(0, 30)).filter(Boolean).slice(0, 8),
  });
  const BATCH = 12;
  const out = [];
  for (let start = 0; start < pngBuffers.length; start += BATCH) {
    const batch = pngBuffers.slice(start, start + BATCH);
    const parts = batch.map((b) => ({ inlineData: { mimeType: "image/png", data: b.toString("base64") } }));
    parts.push({
      text: `Above are ${batch.length} line icons, shown one per image in order (image 1, image 2, …).
Return ONLY a JSON array of EXACTLY ${batch.length} objects, one per image IN THE SAME ORDER:
{ "label": "short-kebab-name", "tags": ["keyword", ...] }
- label: 1-3 lowercase hyphenated words describing what that single icon depicts (e.g. "bar-chart", "person-presenting", "head-idea")
- tags: 3-6 lowercase single-word search keywords
- Exactly one entry per image, in order.`,
    });
    let arr = [];
    try {
      ...
      console.warn(`[Gemini] ordered naming batch ${start} failed:`, e.message);
    }
    for (let i = 0; i < batch.length; i++) out.push(clean(arr[i]));
  }
  return out;
}
...
module.exports = { detectIcons, analyzeLayout, maskRegions, nameIconGrid, nameIconsOrdered, matchIcons };
