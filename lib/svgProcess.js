...
async function extractFromGroups({ svgText, detections = [], layout }) {
  ...
  const topGroups = (ast.children || []).filter((c) => c.name === "g");
  if (topGroups.length === 0) throw new Error("SVG has no top-level groups to segment by");
  ...
  const names = detections.map((d) => ({ label: d.label || "icon", tags: Array.isArray(d.tags) ? d.tags : [] }));
  const used = new Set();
  const candidates = [];
...
module.exports = { extractCandidates, extractFromGroups, classifyElements, elementBounds, parseViewBox, clusterByProximity };
