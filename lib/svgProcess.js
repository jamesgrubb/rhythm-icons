...
async function extractFromGroups({ svgText, detections = [], layout }) {
  ...
  // Groups exist but none have stroke geometry → the icons are outlined/filled
  if (icons.length === 0) {
    throw new Error("These icons look outlined or filled — the library needs stroke-based icons (open paths with a stroke, not filled shapes). In Illustrator, keep the strokes as strokes (don't outline them).");
  }
