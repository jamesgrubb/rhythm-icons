...
// ---------------------------------------------
// Bake a translate+uniform-scale transform into an element's geometry, then
...
// ---------------------------------------------
function bakeTransform(node, scale, tx, ty, effective) {
  const a = { ...node.attributes };
  const s = (n) => String(+(+n).toFixed(3));
  const pt = (x, y) => [Number(x) * scale + tx, Number(y) * scale + ty];

  switch (node.name) {
    case "path":
      if (a.d) a.d = svgpath(a.d).scale(scale).translate(tx, ty).round(3).toString();
      break;
    case "line": {
      const [x1, y1] = pt(a.x1 || 0, a.y1 || 0);
      const [x2, y2] = pt(a.x2 || 0, a.y2 || 0);
      a.x1 = s(x1); a.y1 = s(y1); a.x2 = s(x2); a.y2 = s(y2);
      break;
    }
    case "polyline":
    case "polygon":
      if (a.points) {
        ...
        for (let i = 0; i + 1 < nums.length; i += 2) {
          const [px, py] = pt(nums[i], nums[i + 1]);
          out.push(`${s(px)},${s(py)}`);
        }
        a.points = out.join(" ");
      }
      break;
    case "rect": {
      const [x, y] = pt(a.x || 0, a.y || 0);
      a.x = s(x); a.y = s(y);
      if (a.width != null) a.width = s((Number(a.width) || 0) * scale);
      if (a.height != null) a.height = s((Number(a.height) || 0) * scale);
      if (a.rx != null) a.rx = s((Number(a.rx) || 0) * scale);
      if (a.ry != null) a.ry = s((Number(a.ry) || 0) * scale);
      break;
    }
    case "circle": {
      const [cx, cy] = pt(a.cx || 0, a.cy || 0);
      a.cx = s(cx); a.cy = s(cy); a.r = s((Number(a.r) || 0) * scale);
      break;
    }
    case "ellipse": {
      const [cx, cy] = pt(a.cx || 0, a.cy || 0);
      a.cx = s(cx); a.cy = s(cy);
      a.rx = s((Number(a.rx) || 0) * scale);
      a.ry = s((Number(a.ry) || 0) * scale);
      break;
    }
  }
...
async function extractCandidates({ svgText, detections, pngWidth, pngHeight }) {
  ...
  for (const det of detections) {
    ...
    // Bake the translate+scale into each element's geometry so the icon lands
    // in true 0–24 coordinates with a literal stroke-width of 2 and no
    // transform — identical in shape to the curated library icons, which the
    // PowerPoint theme-recolor path already handles correctly.
    const children = members.map(({ node, effective }) =>
      bakeTransform(node, scale, tx, ty, effective)
    );
