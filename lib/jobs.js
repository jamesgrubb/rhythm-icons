  ...
  if (process.env.DEBUG_INGEST === "1") {
    try {
      finalFields.debug_data = JSON.stringify({
        pngWidth: converted.pngWidth, pngHeight: converted.pngHeight,
        boundingBox: converted.boundingBox, detections,
        pngBase64: converted.pngBuffer.toString("base64"), svgText: converted.svgText,
      });
    } catch (e) { console.warn("[Jobs] debug capture failed:", e.message); }
  }
