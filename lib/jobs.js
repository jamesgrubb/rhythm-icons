...
async function runPipelineFromEps(jobId, epsBuffer) {
  ...
  // 4. Gemini: detect individual icons (bounding boxes) + names + tags
  await updateJob(jobId, { status: "segmenting", progress: 50 });
  const detections = await gemini.detectIcons(converted.pngBuffer);
  // detections: [{ box_2d: [ymin, xmin, ymax, xmax] (0-1000 normalized), label, tags }]
  console.log(`[Jobs] Job ${jobId}: Gemini detected ${detections.length} icon(s)`);

  if (detections.length === 0) {
    throw new Error("Gemini detected no icons in the rendered sheet");
  }

  // 5. Crop per-icon SVGs, normalize to 24x24, classify stroke vs outlined
  ...
  const finalFields = { status: "review_ready", progress: 100, results: candidates };

  // TEMP: capture conversion intermediates for tuning extraction (DEBUG_INGEST=1)
  if (process.env.DEBUG_INGEST === "1") {
    try {
      finalFields.debug_data = JSON.stringify({
        pngWidth: converted.pngWidth,
        pngHeight: converted.pngHeight,
        boundingBox: converted.boundingBox,
        detections,
        pngBase64: converted.pngBuffer.toString("base64"),
        svgText: converted.svgText,
      });
      console.log(`[Jobs] Job ${jobId}: captured debug_data (${(finalFields.debug_data.length / 1024).toFixed(0)} KB)`);
    } catch (e) {
      console.warn("[Jobs] debug capture failed:", e.message);
    }
  }

  await updateJob(jobId, finalFields);
}
