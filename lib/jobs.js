...
async function runIngestJob(jobId, imageId) {
  ...
  // 4. Gemini: detect individual icons (bounding boxes) + names
  ...
  // candidates: [{ name, svg, png_preview, stroke_status: 'valid'|'outlined'|'ambiguous' }]
...
async function getJob(jobId, tenantId) {
  const result = await pool.query(
    `SELECT id, shutterstock_image_id, shutterstock_license_id, status, progress, results, error, created_at, updated_at
     FROM ingest_jobs
...
module.exports = { createIngestJob, getJob, markJobDone, failStaleJobs };
