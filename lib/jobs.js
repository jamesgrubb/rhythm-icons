...
async function runPipelineFromSvg(jobId, svgText) {
  ...
  // 2. Name each EXTRACTED icon as its own ordered image (so the name always
  //    matches the icon — no grid for Gemini to miscount across).
  await updateJob(jobId, { status: "segmenting", progress: 75 });
  try {
    const buffers = renderIconPngs(candidates);
    const names = await gemini.nameIconsOrdered(buffers);
    const used = new Set();
    ...
    console.log(`[Jobs] Job ${jobId}: named ${names.length}/${candidates.length} icons (ordered)`);
  } catch (e) {
    console.warn(`[Jobs] Job ${jobId}: naming failed (${e.message}); icons left with placeholder names`);
  }

  await updateJob(jobId, { status: "review_ready", progress: 100, results: candidates });
}

// Render each candidate icon to its own small PNG buffer (for ordered naming).
function renderIconPngs(candidates) {
  const { Resvg } = require("@resvg/resvg-js");
  return candidates.map((c) => {
    try { return new Resvg(c.svg.replace("<svg ", `<svg width="96" height="96" `), { background: "white" }).render().asPng(); }
