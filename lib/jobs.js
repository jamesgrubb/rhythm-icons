...
async function runPipelineFromSvg(jobId, svgText) {
  ...
  // Render a PNG for Gemini's naming pass (keep the long edge ~2400px)
  await updateJob(jobId, { status: "converting", progress: 30 });
  let png, pngWidth, pngHeight;
  try {
    const r = new Resvg(svgText, { fitTo: { mode: "width", value: 2400 }, background: "white" }).render();
    pngWidth = r.width; pngHeight = r.height; png = r.asPng();
  } catch (e) {
    throw new Error(`Could not render the SVG: ${e.message}`);
  }

  // Gemini naming pass (tiled). Layout pass not needed: branding is filled and
  // dropped by the stroke-only group filter.
  await updateJob(jobId, { status: "segmenting", progress: 55 });
  const detections = await gemini.detectIcons(png, { width: pngWidth, height: pngHeight }).catch((e) => {
    console.warn(`[Jobs] Job ${jobId}: naming failed (${e.message}); continuing unnamed`);
    return [];
  });
  console.log(`[Jobs] Job ${jobId}: SVG groups + ${detections.length} names`);

  // Segment by groups
  await updateJob(jobId, { status: "segmenting", progress: 70 });
  const candidates = await svgProcess.extractFromGroups({ svgText, detections });
